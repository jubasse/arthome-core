...
export const FailureNatureOut: VocabularyOut = vocabularyOut(FAILURE_NATURES).meta({
  // The NAMED member, never the string. The first draft wrote `'unavailable'`
  // here and `arthome-check-enums` refused it inside the minute — a copy of a
  // vocabulary value, in the module that carries that vocabulary to the wire,
  // committed while fixing a neighbouring fault of the same family. The gate
  // does not care who is writing.
