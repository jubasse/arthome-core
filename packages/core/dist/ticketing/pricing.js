...
import { add, money, subtract } from '../money/money.js';
import { applyRate, roundMinor } from '../money/rounding.js';
import { isBefore } from '../time/instant.js';
/** The headline price: the lowest of the ACTIVE tiers. */
export function lowestActivePrice(tiers) {
    const active = tiers.filter((tier) => tier.active);
    return active.reduce((lowest, tier) => lowest === null || tier.amount.amountMinor < lowest.amountMinor ? tier.amount : lowest, null);
}
export function priceOfTier(tiers, tier) {
    return tiers.find((entry) => entry.tier === tier && entry.active)?.amount ?? null;
}
export function activePromotion(promotions, now) {
    return (promotions.find((promotion) => !isBefore(now, promotion.validFrom) && isBefore(now, promotion.validUntil)) ?? null);
}
/**
 ...
 */
export function lateRatePrice(fullPrice, progress) {
 ...
 */
export function applyBestDiscount(basePrice, subscriptionDiscountBps, promotionPrice) {
...
export function serviceFeeFor(schedule, unitPrice, quantity) {
...
export function quoteSeats(unitPrice, quantity, subscriptionDiscountBps, promotionPrice, feeSchedule) {
