 ...
 */
export function disciplinesInEditorialOrder(taxonomy) {
    return [...taxonomy.disciplines].sort((left, right) => left.rank - right.rank);
}
/** A universe's disciplines, in editorial rank. */
export function disciplinesOfFamily(taxonomy, familyId) {
    return disciplinesInEditorialOrder(taxonomy).filter((discipline) => discipline.familyId === familyId);
}
export function findDiscipline(taxonomy, disciplineId) {
 ...
 */
export function matchesTag(tag, term) {
    const needle = normalizeTerm(term);
    if (!needle)
        return false;
    if (normalizeTerm(tag.id) === needle)
        return true;
