...
import { isAvailableIn } from '../catalog/rights.js';
 ...
 */
export const WATCH_FALLBACK_FOR = {
...
function denied(reason, fallback, previewSecondsLeft, validUntil) {
    return {
        allowed: false,
...
export function decideWatch(input) {
    ...
    if (display.state === DisplayState.DRAFT ||
    ...
    if (input.concurrentStreamsOpen >= input.concurrentStreamsAllowed) {
        return denied(WatchDenialReason.CONCURRENT_LIMIT_REACHED, WatchFallbackAction.RELEASE_A_SCREEN, preview, horizon);
    }
    ...
    if (display.state === DisplayState.SCHEDULED) {
        return denied(WatchDenialReason.ROOM_NOT_OPEN, input.holdsSeat ? WatchFallbackAction.NONE : seatAction(input), preview, validUntil);
    }
    // 8. Over, with no replay online.
    return denied(input.timing.replayPolicy === ReplayPolicy.NONE
        ? WatchDenialReason.NO_REPLAY
        : WatchDenialReason.REPLAY_EXPIRED, input.timing.replayPolicy === ReplayPolicy.NONE
        ? WatchFallbackAction.SEE_REPLAY_POLICY
        : WatchFallbackAction.SEE_OTHER_DATES, preview, validUntil);
}
function decideReplay(input, preview, validUntil) {
    const allow = () => ({
        allowed: true,
        scope: WatchScope.FULL,
        reason: null,
    ...
    switch (input.timing.replayPolicy) {
        ...
        case ReplayPolicy.UNIT:
            ...
            return input.replayOnSale
                ? denied(WatchDenialReason.NO_SEAT, seatAction(input), preview, validUntil)
                : denied(WatchDenialReason.REPLAY_NOT_ON_SALE, WatchFallbackAction.SEE_REPLAY_POLICY, preview, validUntil);
        case ReplayPolicy.NONE:
            return denied(WatchDenialReason.NO_REPLAY, WatchFallbackAction.SEE_REPLAY_POLICY, preview, validUntil);
    }
}
/**
 ...
 */
export function concurrentStreamsAllowedFor(planOpenings) {
...
export function previewSecondsLeft(secondsUsed) {
