...
export function orderRankOf(state) {
 ...
 * messages.
 */
export function irreversiblePromiseBlocking(from, to) {
    const reverse = TRANSITIONS.find((transition) => transition.from === to && transition.to === from);
    return reverse?.irreversiblePromiseCode ?? null;
}
export function assertTransitionAllowed(from, to, canDecide) {
...
export function isBlockingChecklistItem(item) {
    return BLOCKING[item];
}
export function publicationReadiness(satisfied) {
