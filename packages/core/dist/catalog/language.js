 ...
 */
export function hasLanguageBarrier(profile) {
 ...
 */
export function isUnderstandable(profile, understoodLanguages) {
    if (profile.dependency === LanguageDependency.NONE)
        return true;
    const understood = new Set(understoodLanguages.map((code) => code.toLowerCase()));
    const covers = (codes) => codes.some((code) => understood.has(code.toLowerCase()));
    if (covers(profile.spoken))
        return true;
    if (covers(profile.subtitles) || covers(profile.surtitles))
        return true;
 ...
 */
export function isLanguageNeutral(profile) {
