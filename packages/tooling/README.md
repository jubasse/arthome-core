...
`arthome-check-tsconfig`, `arthome-check-prettier-conflict`, `arthome-check-language`,
`arthome-check-symbols` and `arthome-check-map`; the generators `arthome-generate-map`, `arthome-generate-agent-map`,
...
pnpm exec arthome-check-symbols            # no warning sign, check mark or emoji committed
...
pnpm exec arthome-comment-density --by block      # rank by longest run, not by excess
...
```
arthome-comment-density: 133 file(s) measured, 2 generated file(s) excluded, 95 above 25% (2113 excess line(s))
  ratio  excess  comment/code   header   longest run  file
    69%     104      163/237      20     34 @L173  packages/core/src/schema/vocabulary.ts
    39%      11        28/71      28            -  packages/tooling/bin/check-map.mjs
```

**The two structural columns matter more than the ratio, because the ratio points at the wrong
files.** The second row above is the demonstration: `check-map.mjs` reads 39 %, and every one of
those lines is its header — `longest run` is empty, so there is no narration in it at all. A ratio
alone would have sent someone to shorten the most valuable prose in the package. Meanwhile forty
lines defending a twelve-line configuration object — the shape §5.10 names — can sit in a file whose
ratio is unremarkable, and `--by block` puts it at the top.

`header` is the run the file opens with (a shebang allowed before it). §5.10 exempts it by name, so
it is reported and never ranked. A run is consecutive comment lines broken by a line of code; a
blank line does not break one, because two paragraphs with a gap are still one stretch of prose to
cross. Both numbers are counted, never interpreted.
...
**What it refuses to detect, and the refusal is the design.** An **orphaned JSDoc** — one left
above nothing after its declaration was deleted — is nearly mechanical, and it is deliberately not
here. Deciding what counts as "a declaration" is a judgement about intent, and a report that shouts
wrongly gets switched off exactly as a gate does (D-024). The stronger reason: an orphaned JSDoc and
a comment claiming a name its file no longer owns are one class — **comments that became false** —
and the second is undetectable by any lexical tool. Shipping a detector for the half that is easy
would advertise coverage of the class, which is `workspace.mjs`'s *"a green gate that checks
nothing"* wearing a different hat. Staleness is found by reading, and by the gates that compare a
claim against its source.

The same applies to the three shapes §5.10 names. A defended default, a comment on a
self-documenting option, and a comment explaining an **absence** are all invisible to a counter —
the last one especially, since there is nothing at the place where it rots.
