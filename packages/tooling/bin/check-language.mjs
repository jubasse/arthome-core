...
/**
 * The line with every string literal's CONTENTS blanked out, same length — leaving
 * the line's STRUCTURE, its real brackets and real comment markers, without the data.
 *
 * Length-preserving because callers index into the original line: deleting the
 * strings would shift every position after the first quote.
 */
function maskStrings(line) {
  return line.replace(/"(?:[^"\\]|\\.)*"/g, (m) => `"${'x'.repeat(Math.max(0, m.length - 2))}"`);
}

function netBracketDepthOutsideStrings(line) {
  let d = 0;
...
/**
 * Source files: the comments only. A French STRING is data — a taxonomy, a fixture, a
 * copy key — and is not this gate's business. A French COMMENT is.
 */
function commentsOfSource(text, ext) {
  ...
  text.split('\n').forEach((raw, i) => {
    ...
    if (jsonProse > 0) {
      out.push([n, line]);
      jsonProse += netBracketDepthOutsideStrings(line);
      return;
    }
    ...
    // This project's JSON prose lives in `_comment` and a few named keys, so those ARE
...
for (const file of files) {
  ...
  // Provenance, never the directory: inside prototypes/ a line lifted verbatim from
  // a mockup is the designer's, not ours.
