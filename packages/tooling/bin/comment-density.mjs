...
/** An extension absent here is not measured at all. */
const SYNTAX = {
  '.ts': { line: ['//'], block: [['/*', '*/']] },
  '.tsx': { line: ['//'], block: [['/*', '*/']] },
  '.mts': { line: ['//'], block: [['/*', '*/']] },
...
function measure(text, syntax) {
  ...
  // A blank line does not break a run: two paragraphs with a gap are still one stretch
  // of prose to cross.
  const runs = [];
  let run = 0;
  let runStart = 0;
...
// Explicit paths skip the walk, so a before/after is produced with THIS code rather
// than a probe that can drift from it.
