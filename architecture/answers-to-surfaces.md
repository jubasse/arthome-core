...
| **25** « Déconnecter cet appareil » coupe-t-il la lecture, et en combien de temps | **Oui — mais jusqu'à 120 s, pas 60. Je t'avais donné un nombre faux.** `identity.device.revoked.v1` est consommé par `streaming`, qui révoque les baux ; le **renouvellement** suivant est refusé (≤ 45 s), mais le **jeton déjà en main** reste valide jusqu'à son expiration, et la périphérie du CDN n'en sait rien. Régime courant 45 à 75 s, pire cas 120 s. **Arbitré : le jeton reste à 120 s** — raccourcir doublerait la fréquence de renouvellement sur le chemin le plus chaud, et le défaut était la promesse, pas la fenêtre. **En pratique tu verras mieux que ça** : un signal `playback:stop` est poussé sur le canal et arrête la lecture en ≤ 2 s — mais c'est une **courtoisie, pas une frontière de sécurité**, et la garantie contractuelle reste 120 s. L'appareil affiche `SIGNED_OUT_ELSEWHERE`, jamais une erreur réseau. | AS §3.3, RT §2 |
...
| # | Réponse | Renvoi |
|---|---|---|
| **9** Qui doit la TVA, sur quelle assiette, qui est redevable | **Instruit, pas supposé.** Recommandation : **modèle « commissionnaire »** — Arthome agit en son nom propre, donc assiette = le **billet entier**, taux = celui du **pays du spectateur**, redevable = **Arthome**. Six indices convergents de la conception l'imposent. **Et la commission porte sur le HT**, pas le TTC, sinon 12 % varieraient avec le pays de l'acheteur. **⚠ C'est une recommandation d'architecture, pas un avis fiscal** : à valider par un conseil avant tout encaissement réel. **Ta trouvaille est retenue, et elle allait plus loin que je ne l'avais vu** : la ventilation est la bonne forme, mais la clé n'est pas le **marché** — c'est la **juridiction**. Un marché de facturation dit dans quelle devise on vend, pas quel taux s'applique ; et le pays ne suffit pas davantage (≈ 9 000 juridictions aux États-Unis). La commande porte désormais une **localisation fiscale d'acheteur avec ses preuves**, et la ligne conserve **le taux appliqué à la vente**. | AP §5.0 |
...
| **Modèle fiscal** (`studio-web` Q9, D5) | **D-015 : modèle commissionnaire acté**, commission sur le HT. **La réserve demeure et figure en tête de l'ADR** : ce n'est pas un avis fiscal, et il doit être validé par un conseil avant tout encaissement réel. La **forme** est gravée et ne dépend pas de cette validation — mais elle a été **durcie** : ventilation par **juridiction** (pas par marché), **localisation fiscale de l'acheteur avec ses preuves** portée par la commande, et **taux appliqué à la vente** conservé sur la ligne. Sans ces faits datés, les reconstituer ne serait pas coûteux : ce serait **impossible**. |
