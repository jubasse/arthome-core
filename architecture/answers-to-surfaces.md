...
> document qui la motive. **Aucune n'est laissée sans réponse** ; celles que je ne peux pas
> satisfaire sont marquées **⚠ NON SATISFAITE** et remontées au chef.
>
...
| **2** Effectif total approximatif à côté du curseur | **Oui.** `track_total_hits: 10000` sur OpenSearch, et le contrat déclare la garantie : **exact jusqu'à 10 000, « au moins 10 000 » au-delà**. La conception est donc tenable sans changement. | CM §14.6 |
...
| **13** Panier : compte ou navigateur | **Compte.** La maquette affiche un panier persistant dans l'en-tête, il se monte sur plusieurs sessions, et les trois storefronts le montrent. Conflit entre appareils : par ligne, dernier écrivain gagne, **rang serveur** (un numéro de version, jamais une date de client). | DM §3.4 |
| **14** Devis opposable, combien de temps | **Oui, 15 minutes.** Le total présenté est celui qui sera débité. Le port est calculé **au devis**, pas à l'ajout. | DM §3.4 |
| **15** Commande de marchandise à deux vendeurs | **Deux commandes.** Motif métier (deux expéditions, deux commissions, deux versements) **et** motif technique indépendant : un `destination charge` Stripe n'admet qu'une seule destination. Le panier se scinde **au paiement**. | AP §3, DM §3.3 |

### Boutique et commerce externe

| # | Réponse | Renvoi |
|---|---|---|
| **16** Commandes passées sur une boutique externe | **Un reflet en lecture seule** (`ExternalOrder`), agrégat distinct avec `external_ref`, `external_host`, `state: external`, `synced_at`. **Ce qu'on garantit** : la fraîcheur au moment de `synced_at`, rien de plus. Quand l'hôte ne répond pas, le reflet est servi **avec son âge**, pas en erreur. Ni facture, ni suivi, ni remboursement chez nous, et le contrat l'assume plutôt que de servir des champs vides. | DM §3.3 |
| **17** Variantes de marchandise | **Oui**, `variants[]` — un t-shirt sans taille n'est pas vendable. Et `labelEn` est une **lacune de donnée** à combler au portage, pas une lacune de traduction (E10). | DM §3.3 |

### Billetterie et accès

| # | Réponse | Renvoi |
|---|---|---|
| **18** Qui émet le code de place | **Le serveur**, toujours. Il s'affiche à l'identique sur trois surfaces : la maquette le calcule par hachage, ce qui donnerait **trois codes différents pour la même place** dès qu'une surface change de fonction. | DM §3.2 |
...
| **25** « Déconnecter cet appareil » coupe-t-il la lecture, et en combien de temps | **Oui, ≤ 60 s** — au prochain renouvellement de jeton. `identity.device_revoked.v1` est consommé par `streaming`, qui révoque les baux de cet appareil. L'appareil visé affiche `SIGNED_OUT_ELSEWHERE`, pas une erreur réseau. | AS §3.3 |
...
| # | Réponse | Renvoi |
|---|---|---|
| **29** Devise d'affichage ≠ devise de facturation | **Elles peuvent différer, et le contrat ne les confond pas** : la devise de **facturation** est une propriété du marché de la date ; la devise d'**affichage** est une préférence de compte. **Mais je recommande de ne pas exercer la seconde au palier 1** : afficher un prix converti qu'on ne peut pas débiter est un mensonge, et D4 montre qu'aucune règle n'a jamais été éprouvée sur deux taux. **⚠ Partiellement non satisfaite** : l'écran de préférences existe, le contrat ne l'honorera pas avant qu'une règle de conversion (taux, date de change, arrondi) soit écrite. Remonté au chef. | AP §5.5 |
...
| **8** Où vit le panier | **Sur le compte.** La maquette affiche un panier persistant, donc un panier local la ferait mentir. | DM §3.4 |
| **9** Commande chez un tiers : reflet et garantie | Reflet **en lecture seule**, servi **avec son âge**. Hors ligne, il s'affiche avec sa date de fraîcheur. Quand l'hôte ne répond pas : l'âge grandit, rien n'échoue. | DM §3.3 |
...
| **8** WHEP sur la coquille native | **Non au palier 5 : le retour de régie WHEP est réservé au studio web ; le studio mobile reçoit LL-HLS**, avec la latence réelle **annoncée** (`monitor_path`) plutôt que promise. Motif : `capacitor://localhost` comme contexte sécurisé dans WKWebView **n'est pas vérifié**, et tu as raison de refuser de le promettre sans appareil réel. **⚠ Partiellement non satisfaite** — la vérification sur appareil reste à faire, et elle conditionne aussi `getUserMedia` et Web Crypto. Remonté au chef. | DM §5.3, AS |
...
| **13** Les six formes absentes : lesquelles entrent au contrat | **Les six entrent, mais pas au même palier.** Ce qui est une **règle** entre dans `@arthome/core` au **palier 1** : seuil de provision (10 000), monotonie des paliers de jauge, fenêtre de priorité (2 h), échéance de révision (72 h). Ce qui est une **forme** entre au contrat `ticketing` au **palier 3**, marqué provisoire : contremarques par catégorie, source d'un article et commandes externes, épinglage en direct. | DM §3.1, §3.3 |

---

## Ce que je n'ai pas pu satisfaire — remonté au chef

Trois points, et un seul est un vrai refus.

1. **La devise d'affichage** (`storefront-web` Q29). L'écran de préférences existe, le contrat ne
   l'honorera pas au palier 1 : afficher un prix converti qu'on ne peut pas débiter est un
   mensonge, et il faudrait d'abord écrire une règle de conversion (source du taux, date de change,
   arrondi, qui porte l'écart). **La préférence doit disparaître de l'interface, ou le chef doit
   commander la règle.** C'est le seul endroit où je demande à la conception de reculer.
2. **Le retour de régie WHEP sur la coquille native** (`studio-mobile` Q8). Reporté au studio web ;
   la vérification de `capacitor://localhost` comme contexte sécurisé dans WKWebView est à faire
   **sur un appareil réel** avant toute promesse, et elle conditionne aussi `getUserMedia` et Web
   Crypto. Ce n'est pas un refus, c'est un « pas avant d'avoir mesuré ».
3. **Le modèle fiscal** (`studio-web` Q9, D5). Je recommande, j'argumente, je grave la **forme**
   qui survit aux deux modèles — mais **je ne tranche pas une question de droit**, et la
   recommandation doit être validée par un conseil avant tout encaissement réel.

Et deux **propositions** qui ne sont pas des constats, et que le chef doit accepter ou refuser :
le **troisième canal de notification** (`in_app` plutôt que `sms`), et la **portée de l'avoir**
(même chaîne, pour borner l'engagement de trésorerie).
